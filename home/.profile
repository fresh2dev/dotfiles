...
export DOTNET_TOOL_BIN_DIR="$HOME/.dotnet/tools"
...
mkdir -p "$MY_TOOL_DIR" "$DOTNET_TOOL_BIN_DIR" "$RUSTUP_HOME" "$CARGO_HOME/bin" "$GOPATH/src" "$GOBIN"
...
export PATH="${MY_TOOL_DIR}:${DOTNET_TOOL_BIN_DIR}:${GOBIN}:${CARGO_HOME}/bin:${DEVBOX_GLOBAL_PREFIX}/bin:${PATH}"
